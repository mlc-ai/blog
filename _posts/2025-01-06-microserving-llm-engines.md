...
To address this gap, we introduce **MicroServing**, a multi-level architecture that provides simple yet effective fine-grained APIs for orchestrating LLM engines. MicroServing introduces simple yet effective REST APIs to support fine-grained sub-request level actions, just like moving toward RISC from CISC. A programmable router transforms user requests into sub-request calls, lifting fine-grained scheduling to the API level, thus enabling the dynamic reconfiguration of different orchestration patterns.
...
class LLMEngine:
  ...
  def prep_recv(prompt, end, seq_id):
     """
     Match prompt[:end] in the context cache of this engine to get the matched prefix length matched_len.
  ...
  def remote_send(prompt, kv_addr_info, recv_rank, begin, end, seq_id):
     """
     Generate KV of prompt[begin:end] on this engine by matching in context cache and prefilling.
  ...
  def start_generate(prompt, begin, seq_id):
     """
     Prefill prompt[begin:] on this engine and start decode on this engine.
